...
class GitHubModelsProvider(BaseAIProvider):
    ...
    Connects to Azure AI inference endpoint for GitHub Models.
    """
    def __init__(self, api_key: str = None, model_id: str = "gpt-4o"):
        resolved_key = api_key or settings.GITHUB_MODELS_API_KEY
    ...
    async def generate_text_stream(self, prompt: str, history: List[Dict[str, Any]]) -> AsyncGenerator[str, None]:
        if not self.api_key:
            yield "GitHub PAT is not configured. Please enter your token in settings."
            return
        ...
        try:
                ...
                async with client.stream("POST", url, headers=headers, json=data, timeout=30.0) as response:
                    if response.status_code != 200:
                        ...
                        try:
                            ...
                            detail = err_msg
                        yield f"\n[GitHub Models API Error {response.status_code}]: {detail}"
                        return
            ...
            yield f"\n[GitHub Models API Exception]: {str(e)}"
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                if res.status_code == 200:
                    ...
                    logger.error(f"GitHub Models list models returned status {res.status_code}: {res.text}")
                    return []
        except Exception as e:
